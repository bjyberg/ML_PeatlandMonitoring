...
library(sf)
library(parallel)
mcluster <- makecluster(detectCores() - 1)
source('TileImages.R')

image_path1 <- '/home/bjyberg/Peatland/data/Site2/S1_dji.tif'
image_path2 <- '/home/bjyberg/Peatland/data/Site2/S2_dji.tif'
...
s1 <- rast(image_path, lyrs = DJI_rast_bands)
s2 <- rast(labels_rast_path)
...
grid <- make_masking_grid(x, Dl_size = 128, factor = 20)



selected_areas <- c(82,68,76,77,39,27,71,52,50,35,74,67,13)
...
x_selected_mask <- mclapply(selected_areas, cut_subset, x,
                          grid, output_folder, identifier ='X', overwrite = T)
...
system.time(lapply(x_selected_mask, function(i) {
...
system.time(mclapply(x_selected_mask, function(i) {
  name <- tools::file_path_sans_ext(basename(sources(i)))
  dl_data_tile(i, n_pixels = 128, output_path = output_folder, site_name = name,
               overwrite = T)
  }
